...
from dataset_guard.policies import visible_splits
...
def export_download(request: HttpRequest) -> StreamingHttpResponse:
    split = get_object_or_404(visible_splits(), pk=request.GET.get("split_id"))
...
def export_path(request: HttpRequest) -> JsonResponse:
    ...
    split = get_object_or_404(visible_splits(), pk=payload.split_id)
    try:
